...
void CBackground::OnInit()
{
	...
	if(Config()->m_ClBackgroundEntities[0] != '\0' && str_comp(Config()->m_ClBackgroundEntities, CURRENT))
		LoadBackground();
}
...
void CBackground::LoadBackground()
{
	...
	str_copy(m_aMapName, Config()->m_ClBackgroundEntities, sizeof(m_aMapName));
	char aBuf[128];
	str_format(aBuf, sizeof(aBuf), "maps/%s", Config()->m_ClBackgroundEntities);
	if(m_pMap->Load(aBuf))
	{
	...
	else if(str_comp(Config()->m_ClBackgroundEntities, CURRENT) == 0)
	{
...
{
	if(str_comp(Config()->m_ClBackgroundEntities, CURRENT) == 0 || str_comp(Config()->m_ClBackgroundEntities, m_aMapName))
	{
		m_LastLoad = 0;
		LoadBackground();
	}
...
void CBackground::OnRender()
{
	...
	if(Config()->m_ClBackgroundEntities[0] != '\0' && str_comp(Config()->m_ClBackgroundEntities, m_aMapName))
		LoadBackground();
	...
	if(Config()->m_ClOverlayEntities != 100)
		return;
