	...
	if(Client()->State() == IClient::STATE_ONLINE || Client()->State() == IClient::STATE_DEMOPLAYBACK)
	{
		...
		else
		{
			...
			if(Config()->m_ClCameraSmoothness > 0)
			{
				...
				{
					s_SpeedBias -= length(m_pClient->m_pControls->m_MousePos[Config()->m_ClDummy] - s_LastMousePos) * log10f(CameraStabilizingFactor) * 0.02f;
			...
			s_LastMousePos = m_pClient->m_pControls->m_MousePos[Config()->m_ClDummy];
			...
			if(Config()->m_ClDynamicCamera && l > 0.0001f) // make sure that this isn't 0
			{
				...
				TargetCameraOffset = normalize(m_pClient->m_pControls->m_MousePos[Config()->m_ClDummy])*OffsetAmount;
			}
