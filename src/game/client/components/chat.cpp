...
void CChat::AddLine(const char *pLine, int ClientID, int Mode, int TargetID)
{
	...
	if(Config()->m_ClShowChat)
	{
		...
		else if(Highlighted || Mode == CHAT_WHISPER)
		{
			...
			{
				m_pClient->m_pSounds->Play(CSounds::CHN_GUI, Config()->m_ClOldChatSounds ? SOUND_CHAT_CLIENT : SOUND_CHAT_HIGHLIGHT, 0);
		...
		else
		{
			...
			{
				m_pClient->m_pSounds->Play(CSounds::CHN_GUI, Config()->m_ClOldChatSounds ? SOUND_CHAT_HIGHLIGHT : SOUND_CHAT_CLIENT, 0);
...
void CChat::OnRender()
{
	...
	else if(m_Mode != CHAT_NONE || m_ChatBufferMode != CHAT_NONE)
	{
		...
		{
			const CGameClient::CClientData& LocalClient = m_pClient->m_aClients[m_pClient->m_LocalClientID[Config()->m_ClDummy]];
	...
	for(int i = StartLine; i < MAX_LINES; i++)
	{
		...
		else if (pLine->m_ClientID >= 0 && Config()->m_ClChatTeamColors && m_pClient->m_Teams.Team(pLine->m_ClientID))
		{
			...
			TextColorName = vec4(rgb.r, rgb.g, rgb.b, 1.f);
		}
		else
