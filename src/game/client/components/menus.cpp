...
bool CMenus::DoButton_GridHeader(const void *pID, const char *pText, bool Checked, int Align, const CUIRect *pRect, int Corners)
{
	...
	UI()->DoLabel(&Label, pText, pRect->h*CUI::ms_FontmodHeight*0.8f, Align|TEXTALIGN_MIDDLE);
...
bool CMenus::DoButton_CheckBox(const void *pID, const char *pText, bool Checked, const CUIRect *pRect, bool Locked)
{
	...
	UI()->DoLabel(&Label, pText, pRect->h*CUI::ms_FontmodHeight*0.8f, TEXTALIGN_ML);
...
void CMenus::RenderLoading(int WorkedAmount)
{
	...
	TextRender()->TextColor(CUI::ms_DefaultTextColor);
	TextRender()->TextSecondaryColor(CUI::ms_DefaultTextOutlineColor);
	UI()->DoLabel(&Rect, "F-Client", 48.0f, TEXTALIGN_CENTER);
	...
	if(m_Popup == POPUP_NONE)
	{
		...
		else
		{
			...
			// render top buttons
			{
				...
				UI()->DoLabel(&XText, "\xE2\x9C\x95", XText.h*CUI::ms_FontmodHeight, TEXTALIGN_CENTER);
