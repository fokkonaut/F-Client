...
void CMenus::RenderServerbrowserOverlay()
{
	...
	if(Type == CInfoOverlay::OVERLAY_HEADERINFO)
	{
		...
		// render background
		View.Draw(vec4(0.25f, 0.25f, 0.25f, 1.0f), 6.0f);
		...
		UI()->DoLabel(&NumServers, aBuf, FontSize, TEXTALIGN_CENTER);

		str_format(aBuf, sizeof(aBuf), "%s: %d", Localize("Players"), pFilter->NumPlayers());
		UI()->DoLabel(&NumPlayers, aBuf, FontSize, TEXTALIGN_CENTER);
	}
	...
	else if(Type == CInfoOverlay::OVERLAY_PLAYERSINFO)
	{
		...
		if(pInfo && pInfo->m_NumClients)
		{
			...
			// render background
			View.Draw(vec4(1.0f, 1.0f, 1.0f, 0.75f), 6.0f);
			...
			for (int i = 0; i < pInfo->m_NumClients; i++)
			{
				...
				Name.Draw(BackgroundColor, 4.0f);
				...
				// name
				UI()->DoLabelHighlighted(&Name, pInfo->m_aClients[i].m_aName, Config()->m_BrFilterString, FontSize, CUI::ms_DefaultTextColor, CUI::ms_HighlightTextColor, TEXTALIGN_ML);

				// clan
				UI()->DoLabelHighlighted(&Clan, pInfo->m_aClients[i].m_aClan, Config()->m_BrFilterString, FontSize, CUI::ms_DefaultTextColor, CUI::ms_HighlightTextColor, TEXTALIGN_ML);
		...
		else
		{
			...
			UI()->DoLabel(&View, Localize("no players", "server browser message"), View.h*CUI::ms_FontmodHeight*0.8f, TEXTALIGN_MC);
		}
