...
void CRenderTools::Init(CConfig *pConfig, IGraphics *pGraphics, ITextRender *pTextRender)
{
	m_pConfig = pConfig;
...
void CRenderTools::RenderTee(CAnimState *pAnim, const CTeeRenderInfo *pInfo, int Emote, vec2 Dir, vec2 Pos, bool SpecChar)
{
	vec2 Direction = Dir;
	vec2 Position = Pos;
	...
	for(int p = 0; p < 2; p++)
	{
		...
		for(int f = 0; f < 2; f++)
		{
			...
			if(f == 1)
			{
				...
				// draw marking
				if(pInfo->m_aTextures[SKINPART_MARKING].IsValid() && !OutLine && !SpecChar)
				{
